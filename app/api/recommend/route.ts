...
export async function POST(request: NextRequest) {
  ...
  if (!item_label || value_range === undefined || shippable === undefined) {
    return NextResponse.json(
      { error: "Missing required fields: item_label, value_range, shippable" },
      { status: 400 }
    );
  }

  const systemPrompt = `You are a warm, thoughtful advisor for GoShed — an app that helps people decide what to do with things they own (gift, donate, sell, keep, or trash). Given an item and the person's life context, give one clear recommendation. Respond with a valid JSON object only (no markdown, no extra text) with exactly these keys:
- recommendation: exactly one of "gift", "donate", "sell", "keep", "trash"
- reason: a short, warm, personal explanation (1-3 sentences) that ties the recommendation to their life context
- next_step: one concrete, actionable next step they can take right away`;
...
Give one recommendation (gift/donate/sell/keep/trash) with a warm personal reason and one concrete next step. Return only the JSON object.`;
  ...
  const validRecommendations = ["gift", "donate", "sell", "keep", "trash"] as const;
